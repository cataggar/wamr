...
//! #890 (aarch64 backend parity), #891 (JitCodeCache integration), #892
//! (deferred IR-pass pipeline), and #894 (concurrent first-call
...
test "#888 lazy-JIT: call_indirect compiles through a stable trampoline" {
    ...
    try std.testing.expectEqual(aot_runtime_mod.LazyJitState.SlotState.pending, fixture.inst.lazy_jit.slotState(0));
    ...
    try std.testing.expectEqual(aot_runtime_mod.LazyJitState.SlotState.ready, fixture.inst.lazy_jit.slotState(0));
...
//   (func $never_called (export "never_called") (param i32) (result i32)
//     local.get 0 i32.const 1 i32.sub))
const lazy_phi_fixture_wasm = [_]u8{
...
fn deinitLazyOut892(allocator: std.mem.Allocator, out: *component_aot_compile.LazyJitOut) void {
    out.ir_module.deinit();
    allocator.free(out.lazy_local_indices);
    if (out.needs_trampoline.len > 0) allocator.free(out.needs_trampoline);
    out.* = .{};
}
...
fn measureCompileMinNs(
    ...
    while (iter < 3) : (iter += 1) {
        if (opts.lazy_jit) {
            ...
            deinitLazyOut892(allocator, &lazy_out);
        } else {
            const start_ns = nowNs();
...
test "#892 lazy-JIT: eager pipeline skips lazy functions and helper replays it later" {
    ...
    const module = try core_loader_mod.load(&lazy_phi_fixture_wasm, arena.allocator());
    var ir_module = try frontend_mod.lowerModule(&module, gpa);
    defer ir_module.deinit();

    var eligibility = try lazy_jit.findLazyEligibleFunctions(&module, &ir_module, .x86_64, gpa);
    defer eligibility.deinit(gpa);
    const lazy_skip = eligibility.eligible;
...
test "#892 lazy-JIT: deferred functions compile correctly on first call after skipped eager passes" {
    if (comptime !config.lazy_jit) return error.SkipZigTest;
    if (comptime !can_exec_aot) return error.SkipZigTest;
    // The `via_funcptrs` non-leaf stub path and trampoline mechanism are
    // x86_64-only; keep this x86_64-scoped like #892's own tests were.
    if (comptime builtin.cpu.arch != .x86_64) return error.SkipZigTest;
    ...
    const cwasm = try component_aot_compile.compileCoreWasmCached(
        gpa,
        &lazy_phi_fixture_wasm,
        .{ .lazy_jit = true },
        .{ .lazy_jit_out = &lazy_out },
    );
    defer gpa.free(cwasm);
    var lazy_out_owned_by_driver = false;
    defer if (!lazy_out_owned_by_driver) deinitLazyOut892(gpa, &lazy_out);
    ...
    try std.testing.expectEqual(aot_runtime_mod.LazyJitState.SlotState.pending, inst.lazy_jit.slotState(0));
    try std.testing.expectEqual(aot_runtime_mod.LazyJitState.SlotState.pending, inst.lazy_jit.slotState(1));
    try std.testing.expectEqual(aot_runtime_mod.LazyJitState.SlotState.pending, inst.lazy_jit.slotState(2));
    try std.testing.expectEqual(aot_runtime_mod.LazyJitState.SlotState.pending, inst.lazy_jit.slotState(3));
    ...
    // add1, phi_local, and mul2 are no longer pending; whichever local
    // index corresponds to `never_called` (the one we never invoked) must
    // still be pending -- proving its compilation was genuinely skipped,
    // not merely done in a different order.
    try std.testing.expectEqual(aot_runtime_mod.LazyJitState.SlotState.ready, inst.lazy_jit.slotState(0));
    try std.testing.expectEqual(aot_runtime_mod.LazyJitState.SlotState.ready, inst.lazy_jit.slotState(1));
    try std.testing.expectEqual(aot_runtime_mod.LazyJitState.SlotState.ready, inst.lazy_jit.slotState(2));
    try std.testing.expectEqual(aot_runtime_mod.LazyJitState.SlotState.pending, inst.lazy_jit.slotState(3));

    // Second call to add1 must reuse the already-compiled code (not
    // recompile) -- verified indirectly: the slot stays `.ready` and the
