...
pub const CodeBuffer = struct {
    ...
    /// AND Wd, Wn, #0x7 — mask a scalar i8x16 shift count modulo 8.
    pub fn andImm32Mask7(self: *CodeBuffer, rd: Reg, rn: Reg) !void {
        try self.emit32(0x12000800 | (@as(u32, rn.encoding()) << 5) | rd.encoding());
    }
    ...
    const Ld1LaneWidth = enum { b, h, s, d };

    fn ld1Lane(self: *CodeBuffer, vt: u5, width: Ld1LaneWidth, lane: u8, rn: Reg) !void {
    ...
    /// LD1 { Vt.B }[lane], [Xn] — load one byte lane, preserving other lanes.
    pub fn ld1LaneB(self: *CodeBuffer, vt: u5, lane: u4, rn: Reg) !void {
        try self.ld1Lane(vt, .b, lane, rn);
    }

    /// LD1 { Vt.H }[lane], [Xn] — load one halfword lane, preserving other lanes.
    pub fn ld1LaneH(self: *CodeBuffer, vt: u5, lane: u3, rn: Reg) !void {
        try self.ld1Lane(vt, .h, lane, rn);
    }

    /// LD1 { Vt.S }[lane], [Xn] — load one word lane, preserving other lanes.
    pub fn ld1LaneS(self: *CodeBuffer, vt: u5, lane: u2, rn: Reg) !void {
        try self.ld1Lane(vt, .s, lane, rn);
    }

    /// LD1 { Vt.D }[lane], [Xn] — load one doubleword lane, preserving other lanes.
    pub fn ld1LaneD(self: *CodeBuffer, vt: u5, lane: u1, rn: Reg) !void {
    ...
    /// FABS Vd.4S, Vn.4S.
    pub fn fabs4s(self: *CodeBuffer, vd: u5, vn: u5) !void {
        return self.f32x4UnOp(.abs, vd, vn);
    }

    /// FNEG Vd.4S, Vn.4S.
    pub fn fneg4s(self: *CodeBuffer, vd: u5, vn: u5) !void {
        return self.f32x4UnOp(.neg, vd, vn);
    }
    ...
    /// SSHL Vd.16B, Vn.16B, Vm.16B — signed variable shift.
    pub fn sshl16b(self: *CodeBuffer, vd: u5, vn: u5, vm: u5) !void {
        try self.emit32(0x4E204400 |
            (@as(u32, vm) << 16) |
            (@as(u32, vn) << 5) |
            vd);
    }

    /// USHL Vd.16B, Vn.16B, Vm.16B — unsigned variable shift.
    ...
    /// NEG Vd.16B, Vn.16B.
    pub fn neg16b(self: *CodeBuffer, vd: u5, vn: u5) !void {
        try self.emit32(0x6E20B800 |
            (@as(u32, vn) << 5) |
            vd);
    }
...
test "emit: AND w3, w4, #7" {
    var code = CodeBuffer.init(std.testing.allocator);
    defer code.deinit();
    try code.andImm32Mask7(.x3, .x4);
    try expectWord(0x12000883, &code);
}
...
test "emit: LD1 lane loads" {
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
        try code.ld1LaneB(0, 0, .x1);
        try expectWord(0x0D400020, &code);
    }
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
        try code.ld1LaneB(0, 15, .x1);
        try expectWord(0x4D401C20, &code);
    }
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
        try code.ld1LaneH(2, 0, .x3);
        try expectWord(0x0D404062, &code);
    }
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
        try code.ld1LaneH(2, 7, .x3);
        try expectWord(0x4D405862, &code);
    }
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
        try code.ld1LaneS(4, 0, .x5);
        try expectWord(0x0D4080A4, &code);
    }
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
        try code.ld1LaneS(4, 3, .x5);
        try expectWord(0x4D4090A4, &code);
    }
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
        try code.ld1LaneD(6, 0, .x7);
        try expectWord(0x0D4084E6, &code);
    }
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
...
test "emit: f32x4 vector unary ops" {
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
        try code.fabs4s(16, 17);
        try expectWord(0x4EA0FA30, &code);
    }
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
        try code.fneg4s(16, 17);
        try expectWord(0x6EA0FA30, &code);
    }
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
...
test "emit: i8x16 variable shifts" {
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
        try code.sshl16b(16, 17, 30);
        try expectWord(0x4E3E4630, &code);
    }
    {
        var code = CodeBuffer.init(std.testing.allocator);
        defer code.deinit();
