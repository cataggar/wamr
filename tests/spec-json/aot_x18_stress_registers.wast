...
(module
  ...
  (func (export "run") (result i64)
    (call $stress_registers (i64.const 42))
  )
)

;; Smoke-test the register-allocator path: just invoke `run` and confirm
;; it produces the deterministic sum derived from the seed `i64.const 42`.
;; The original macOS-arm64 reproducer looped 1000 iterations; the
;; correctness signal here is that the AOT backend lays out the 24 i64
;; locals + final reduction without crashing or returning a wrong value.
(assert_return (invoke "run") (i64.const 96633721459010))
