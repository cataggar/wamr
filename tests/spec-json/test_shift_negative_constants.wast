...
;; Trap-on-mismatch: each helper above traps via `unreachable` if the shift
;; result disagrees with the expected constant. The invoke succeeds only
;; when every comparison holds, so a single (assert_return ...) covers all
;; six cases.
