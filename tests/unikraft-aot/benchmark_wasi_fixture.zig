...
export fn args_environment_output() void {
    ...
    if (args_sizes_get(&count, &size) != 0 or count != 2 or size != 13) unreachable;
    if (args_get(&pointers, &bytes) != 0) unreachable;
    if (!equal(bytes[0..13], "fixture\x00arg1\x00")) unreachable;
    if (environ_sizes_get(&count, &size) != 0 or count != 1 or size != 4) unreachable;
    if (environ_get(&pointers, &bytes) != 0) unreachable;
    if (!equal(bytes[0..4], "A=B\x00")) unreachable;
    const iov = [_]Iovec{ .{ .pointer = "one", .length = 3 }, .{ .pointer = "two", .length = 3 } };
    var written: u32 = 0;
    if (fd_write(1, &iov, 2, &written) != 0 or written != 6) unreachable;
    if (fd_write(2, &iov, 2, &written) != 0 or written != 6) unreachable;
}
