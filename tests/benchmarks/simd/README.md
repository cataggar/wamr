...
The `simd_extend_lowhigh_4k_loop` row exercises the 12 widening lane-extension operations. It walks the same 4 KiB byte and halfword input shape, runs `i16x8.extend_low/high_i8x16_s/u`, `i32x4.extend_low/high_i16x8_s/u`, and `i64x2.extend_low/high_i32x4_s/u`, combines the results with `v128.xor`, stores the output, and returns one scalar checksum lane.

The `simd_extmul_lowhigh_4k_loop` row exercises the 12 widening lane-multiply operations. It walks paired 4 KiB byte, halfword, and word input streams, runs `i16x8.extmul_low/high_i8x16_s/u`, `i32x4.extmul_low/high_i16x8_s/u`, and `i64x2.extmul_low/high_i32x4_s/u`, combines the results with `v128.xor`, stores the output, and returns one scalar checksum lane.
...
The small `simd_i32x4_*_lane0`, `simd_i16x8_*_lane0`, `simd_i8x16_*`, `simd_i64x2_*`, `simd_v128_load*`, `simd_f32x4_convert_*`, and `simd_f64x2_convert_*` rows are coverage/status probes for individual opcode families. They intentionally return one scalar lane so interpreter, AOT, and optional Wasmtime rows can be compared before the runtime supports direct exported v128 values. The i16x8 and i8x16 comparison, replace-lane, and widening-load rows cover signed vs unsigned extraction of all-ones masks and high-bit lane values; the i64x2 and f64x2 rows wrap extracted i64 lanes to i32 for the exported checksum.
