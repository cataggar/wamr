...
Status: **NARROW PROTOTYPE** (leaf functions only; both x86_64 and aarch64 as of #879 — see [Scope](#scope) and [Deferred to follow-up work](#deferred-to-follow-up-work)).
...
`PrecompileOptions` gained a `lazy_jit: bool` opt-in flag and `CompileCacheCtx` gained a `lazy_jit_out: ?*LazyJitOut`. When both are set, `compileCoreWasmCached` computes the eligible set, threads it into the target backend's codegen call (x86_64 or aarch64 — `LazyJitOut.target_arch` records which, since this is an in-process JIT feature and compiled code always executes in the same process it was compiled in), and — instead of freeing the lowered IR module at the end like it normally does — **moves it** into the caller-supplied `LazyJitOut` (along with the compact list of deferred local indices), so the deferred functions' IR survives past the call for later on-demand compilation.
...
**Why type-erased?** `AotInstance`/`runtime.zig` must not depend on compiler types directly — the plain (non-`-Djit`) `wamr` binary links no compiler at all (#695's whole point). The actual "compile function N" logic — which needs `ir.IrFunction`, the target backend's codegen module (x86_64 or aarch64), etc. — lives in `src/component/aot_compile.zig`'s new `LazyCompileDriver`, which owns the retained `LazyJitOut` and is wired into an `AotInstance` via the also-new `setupLazyJit(inst, lazy_out, allocator)` helper. This exactly mirrors the existing `TrampolinePool.ctx: *anyopaque` pattern.
...
`src/tests/lazy_jit_spike_test.zig` (gated on `config.lazy_jit`, `x86_64`/`aarch64` as of #879):
...
2. **Measured effect**: a 200-leaf-function synthetic module (`fN(x) = x + N`, no calls, no tables — all 200 eligible) compiled with `opts.lazy_jit = true` vs. the eager default. Measured (this repo's benchmark hardware, x86_64, ReleaseFast, 2026-07-10):
   ...
   #879 re-ran the same correctness + measured-effect tests cross-compiled for `aarch64-linux-musl` and executed under `qemu-aarch64-static` (no physical aarch64 hardware was available in that session either, matching #874's constraint) — both tests pass, and the same qualitative eager-vs-lazy reduction shows up (absolute µs figures aren't meaningful under emulation, so not reproduced here; they're heavily dominated by qemu's binary-translation overhead, not real aarch64 silicon).
...
- ~~**aarch64.**~~ **Done (#879):** `CompileOptions.lazy_skip` and a `compileFunctionForLazyJit` per-function entry point were ported to `aarch64/compile.zig`, `LazyJitOut` gained a `target_arch` field so `LazyCompileDriver.compileFn` dispatches to the right backend, and the `-Djit=true -Dlazy_jit=true` guard restricting `PrecompileOptions.lazy_jit` to `.x86_64` was removed. Verified by cross-compiling for `aarch64-linux-musl` and running the full test suite (including `lazy_jit_spike_test.zig`, whose `can_exec_aot` gate now includes aarch64) under `qemu-aarch64-static` — no physical aarch64 hardware was available in that session either, matching #874's constraint, but this is a real (if emulated) execution of the generated aarch64 machine code, not just a compile check.
