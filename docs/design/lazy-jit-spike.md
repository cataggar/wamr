...
- ~~**Interaction with the JIT code cache (#857).**~~ **Done (#879):** `LazyCompileDriver.compileFn` now calls `JitCodeCache.checkBudget`/`register` before/after its `platform.mapExecutableCode` call, and `LazyJitState.free` calls `JitCodeCache.unregister` for each lazily-compiled mapping on `AotInstance.destroy()` — see `src/tests/lazy_jit_spike_test.zig`'s `"#879 lazy-JIT spike: lazily-compiled functions register with JitCodeCache and unregister on destroy"` test.
