...
**Update (#879):** the IR-optimization-passes gap above is now closed too — see the corresponding entry in [Deferred to follow-up work](#deferred-to-follow-up-work).
...
- ~~**Skipping IR-optimization passes for lazy functions**, not just codegen (see the note above).~~ **Done (#879):** `passes.RunOptions` gained a `lazy_skip: []const bool` field (same indexing/semantics as the codegen-level field of the same name); `runPassesWithOptionsScoped`'s per-function loop skips SSA promotion, the pass fixpoint, `scrubUnreachableBlocks`, and their verify calls entirely for lazy-eligible functions, leaving their IR exactly as `frontend.lowerModule` produced it. Safe because running codegen against un-optimized (but still lowered) IR is already a supported path (`-O0` skips this whole pipeline module-wide). `compileCoreWasmCached` threads the same `lazy_skip` slice it already computes into both this and the codegen call.
