...
Status: **LEAF-ONLY PROTOTYPE** (x86_64 lazy trampolines now cover exported direct calls plus `call_indirect` / `ref.func` / `call_ref` for trampoline-compatible leaf signatures; see [Scope](#scope) and [Deferred to follow-up work](#deferred-to-follow-up-work)).
...
3. **Trampoline-compatible signature**: the deferred function's lowered ABI must fit the current trampoline envelope (x86_64 only, no retptr, no `v128`, at most one scalar result, at most 9 user args after `vmctx`).
...
The trampoline dispatcher checks `pending[local_idx]` before resolving the call target; if true, it invokes `compile_fn` (which compiles the one function via the same real per-function codegen entry point the eager path uses, maps it executable via `platform.mapExecutableCode`, and returns the address+size), stores the result, clears `pending`, and then forwards through the compiled native code. `AotInstance.destroy()` frees the tracking arrays, trampoline pool, and `munmap`s every compiled function's own mapping.
...
**Scope of what this hook covers**: exported direct calls (`callFunc` / `callFuncScalar`) plus x86_64 `call_indirect` / `ref.func` / `call_ref` that consume the published trampoline pointer. Non-leaf direct-call-target functions still stay eager.
...
2. **Table / funcref reachability**: x86_64 lazy-JIT tests now cover an active-element `call_indirect` path and a `ref.func + table.set + call_indirect` path, both compiling a leaf target through its stable trampoline on first use while leaving `inst.funcptrs[target]` unchanged before and after compilation.
3. **Measured effect**: a 200-leaf-function synthetic module (`fN(x) = x + N`, no calls, no tables — all 200 eligible) compiled with `opts.lazy_jit = true` vs. the eager default. Measured (this repo's benchmark hardware, x86_64, ReleaseFast, 2026-07-10):
