...
Status: **x86_64/aarch64 PROTOTYPE** (non-leaf direct-call graphs via stable entry stubs on x86_64 (#887); `call_indirect` / `ref.func` / `call_ref` for trampoline-compatible leaf signatures via native trampolines, x86_64 only (#888); leaf-only stub emission also implemented on aarch64 (#890); see [Scope](#scope) and [Deferred to follow-up work](#deferred-to-follow-up-work)).
...
Given the two problems above, this prototype now supports two complementary mechanisms, and every lazy-eligible local function uses exactly one of them (`src/compiler/lazy_jit.zig`'s `findLazyEligibleFunctions` decides both eligibility and, for eligible functions, which mechanism applies):

1. **Stub mechanism (#887)**: for functions that are never reachable through a table, `ref.func`, or `call_indirect`/`call_ref` — i.e. only ever entered via `callFuncScalar` or a direct intra-module `.call` — a small, always-resident entry stub is emitted at the function's normal text-section offset instead of the real body. Direct `.call`/tail-call patches resolve against the stub's offset exactly as if the function had been compiled eagerly; the stub calls back into the runtime to compile the real body on first entry, then forwards through it. Non-leaf functions are fine here (their own lazy body lowers local direct calls indirectly through `vmctx.funcptrs_ptr` instead of relying on inter-function rel32 patches, so it never depends on some other not-yet-compiled function's offset).
...
`compileModuleCachedWithOptions` (on both native backends now — x86_64 and aarch64) gained an additive `CompileOptions.lazy_skip: []const bool` field (indexed by local function index, empty by default — zero behavior change for every existing caller). For an index where `lazy_skip[i]` is true, the per-function loop writes a **stable entry stub** (#887; aarch64 parity via #890) instead of calling into the real per-function codegen — safe for both mechanisms above: stub-mechanism functions are entered directly through it, and trampoline-mechanism functions (x86_64 only) never execute it at all (their published address is always the trampoline, never the text-section offset), since they're never a direct `.call` target. The aarch64 backend only ever sees leaf, never-directly-called functions here (`findLazyEligibleFunctions` restricts non-leaf stub eligibility to x86_64, since aarch64 has no `via_funcptrs`-style indirect local-call lowering yet).
...
6. **Measured effect**: a 200-leaf-function synthetic module (`fN(x) = x + N`, no calls, no tables — all 200 eligible) compiled with `opts.lazy_jit = true` vs. the eager default. Measured (this repo's benchmark hardware, x86_64, ReleaseFast, 2026-07-10):
...
- **Non-leaf functions and the trampoline mechanism on aarch64.** #890 ports the original leaf-only stub emission to the aarch64 backend and validates it on real Apple Silicon hardware, but #887's non-leaf `via_funcptrs` local-call lowering and #888's native trampolines remain x86_64-only; `findLazyEligibleFunctions` keeps aarch64 restricted to leaf, never-directly-called functions accordingly.
