...
- ~~**Thread-safety of concurrent first-calls to the *same* lazy function.**~~ **Done (#879):** `LazyJitState` gained a `mutex: platform.Mutex` field; `callFuncScalar` now does an unsynchronized `.acquire`-load fast-path peek at `pending[local_idx]`, and only takes the lock (with a double-checked re-read of `pending`) when that peek says "maybe still pending". Only one thread ever calls `compile_fn` for a given `local_idx`; every other racing thread sees the lock-protected re-check come back `false` and falls through to reading the already-stored `compiled[local_idx]`. See `src/tests/lazy_jit_spike_test.zig`'s `"#879 lazy-JIT spike: concurrent first-calls to the same lazy function compile exactly once"` stress test.
