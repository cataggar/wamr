...
- **`call_indirect`/`ref.func` reachable functions.** Needs the native-trampoline approach sketched above (extending `TrampolinePool`/`host_trampolines.zig`'s dispatch machinery to plain `AotInstance`, or a dedicated lazy-compile trampoline kind), so `func_addrs[i]` can safely be "not yet compiled" for anything the runtime might jump to directly.
  ...
  **Still open (phase 2):** wiring `findLazyEligibleWithTrampoline` and `lazy_call_trampoline.zig` into `compileCoreWasmCached`/`mapCodeExecutable`/`AotInstance` so a real `call_indirect`/`call_ref` actually exercises this path end-to-end. Deliberately left as a separate follow-up: `mapCodeExecutable`'s `func_addrs`/table-population logic is already large and correctness-critical, and phase 1's job was to get the two genuinely new pieces (the relaxed eligibility analysis, and the trampoline machine code) independently right first.
