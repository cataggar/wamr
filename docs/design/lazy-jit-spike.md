...
- **Components.** `precompileComponentInMemory` compiles each core module independently already, so the per-core mechanism here composes in principle, but the `LazyJitOut`/`LazyCompileDriver` plumbing only threads through the single-core `compileCoreWasmCached` path today.
  ...
  **Still open (phase 2, follow-up, not done in this change):** nothing in `component/instance.zig`'s per-core instantiation loop (`instantiateWithOptions`) reads `lazy_jit_outs` yet, so there's no way today to actually get a `-Djit=true` component (`wamr run`/`wamr serve` with no precompiled manifest) to defer any of its functions end-to-end — `compileCoreWasmCached`'s `PrecompileOptions.lazy_jit` is real and tested at the compile layer, but nothing in the CLI or the runtime's component-instantiation path opts into it yet. `instantiateWithOptions` is a large, correctness-critical function (AOT/interpreter backend fallback, section-aware core-instance walking) that deserves its own focused pass, the same way `mapCodeExecutable`'s wiring got its own phase in the call_indirect/ref.func work above.
