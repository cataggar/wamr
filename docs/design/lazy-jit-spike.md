...
Status: **x86_64 PROTOTYPE** (covers non-leaf direct-call graphs via stable entry stubs (#887), plus `call_indirect` / `ref.func` / `call_ref` for trampoline-compatible leaf signatures via native trampolines (#888); see [Scope](#scope) and [Deferred to follow-up work](#deferred-to-follow-up-work)).
...
`compileModuleCachedWithOptions` (both backends conceptually; **only x86_64 is wired up in this spike** — see [Scope](#scope)) gained an additive `CompileOptions.lazy_skip: []const bool` field (indexed by local function index, empty by default — zero behavior change for every existing caller). For an index where `lazy_skip[i]` is true, the per-function loop writes a **stable entry stub** (#887) instead of calling into the real per-function codegen — safe for both mechanisms above: stub-mechanism functions are entered directly through it, and trampoline-mechanism functions never execute it at all (their published address is always the trampoline, never the text-section offset), since they're never a direct `.call` target.
