...
Measurement uses the host-owned **version 2 / `wamr-embedding-v2`** contract.
The exact `snapshot-replay` lifecycle must match the request, target and independent
image receipt. Snapshot creation, each reset and each invocation have separate
actual timings. Version 1 and warm-instance lifecycle declarations are rejected;
restored-state replay is never published as warm steady state. Qualified deployment
inputs remain mandatory; enabling this software does not qualify hardware or images.
...
An optional final fault name injects a **correctness-only negative-test failure**,
never a measurement capability. Supported names are `opening-clock`,
`closing-clock`, `backwards-clock`, `report-oom`, `output-limit`, `reset-clock`,
`reset-backwards-clock`, `reset-protect`, `steady-opening-clock`,
`steady-closing-clock` and `steady-output-limit`. They exercise real guest calls,
capture buffers and protection transitions. An opening-clock failure prevents the
call; a closing-clock or later allocation failure retains the actual terminal/output.
...
* **Lifecycle setup:** snapshot construction after successful instantiation and
  module start, before the first invocation, including allocation and copying.
...
* **Repeated invocations:** the **same instance and native code** are called again
...
The reusable `src/bench/native_runner.zig` implements
`reset_policy: restore-post-start-snapshot`. Its exact sorted `reset_scope` is
...
`table-entries-signatures`, `wasi-context`. Before each repeat,
...
are rejected before restoration rather than silently approximated. A fresh WASI
Context restores the original descriptors and sticky exit state; output buffers
clear while retaining capacity. Pending output errors prevent reset before any
state mutation, so context replacement cannot erase an unreported failure.
The minimal adapter acquires no external files or sockets.

Code/import pointers, reservations, immutable artifact bytes and completed
module-start state are reused. The module start section is not rerun. Native
pending-terminal and per-call execution setup occur inside the timed API call;
reset does not copy a continuation or overwrite that private execution state.
This producer-specific boundary is covered by memory/global/table/drop-state
mutation, growth, descriptor/exit, partial-protection and resource-failure
regressions as well as exact repeated CoreMark CRCs. It is not a generic runtime
checkpoint API. Reset/snapshot costs are outside invocation timing; do not
...
`phases.lifecycle_setup_ticks` records snapshot creation. Each attempted reset
has a `reset_events` entry containing its 2-based `before_invocation`, actual
`outcome` and optional `elapsed_ticks`, plus an allocation-free private
`WAMR_NATIVE_RESET=` diagnostic. Failed resets stop before another call; a
completed reset may also be the final event when the next opening clock fails.
Neither case invents an invocation. Host reports separate snapshot call-only,
reset and reset-plus-call distributions; `steady_state_seconds` remains null
for snapshot replay.
...
A failed/backwards closing clock leaves the actual invocation duration null and
adds `post-call-clock` to `measurement_errors`, preserving the terminal and bytes
in the failed v2 result. Incomplete stdout remains canonical base64 of the known
prefix with `stdout_complete: false` and `stdout-copy`; callback/pending write
errors add `pending-output`. The error list is sorted. Such failures stop further
calls or resets, and never contribute successful timing statistics.
Later report-allocation failure cannot erase the already-written private record.
If report serialization is impossible, the producer fails without fabricating a
duration or silently omitting the terminal from private evidence. Existing host capture
...
Only the exact version 2 lifecycle is accepted by the setup below.
...
| Field | Meaning |
|---|---|
| `schema_version` | `1` for this local deployment configuration; native receipts/results use `2` |
...
The receipt must declare the exact `execution_lifecycle` above,
`runtime_linkage: static` and all required safety
...
request strings or proved by hashing a local image file. Protocol v2's `observed`
...
The per-field origin is fixed, not inferred from the word `observed`: CPU model
and active CPU count come from Linux; runtime/AOT/wasm/compiler/image byte hashes
come from local file reads; SKU/region, source lineage, build settings, configured
RAM and boot-to-image relationship come from the independent qualified receipt.
...
incompatible meanings. Protocol v2 has no allocation-byte sample field: its
