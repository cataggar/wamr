...
class ThreadBenchmarkTests(unittest.TestCase):
    ...
    def test_exact_sizing_formula_and_rounding(self) -> None:
        ...
        override = args.pilot_iteration_plan["aot"]["hot"]["1"]
        self.assertEqual(override, 1_234)
        self.assertEqual(
            bench.selected_iterations_from_elapsed(
                override, 2 * bench.PROJECTED_EVIDENCE_MINIMUM_NS
            ),
            (617, 617),
        )
        algorithm = bench.sizing_algorithm_spec(90)
        self.assertEqual(
    ...
    def test_general_envelope_and_declared_stronger_cell_envelope(self) -> None:
        wait_target = bench.sizing_candidates_for_cell(
            ...
            [
                {
                    ...
                    "required_iterations": 15_000_000,
                    "rounded_iterations": 15_000_000,
                }
            ],
        )
        self.assertEqual(wait_target["selected_iterations"], 15_000_000)
        hot_target = bench.sizing_candidates_for_cell(
            "aot", "hot", 8, 450_000_000, 2_171_788_000
        )
        self.assertEqual(
            hot_target["general_rounded_iterations"], 4_150_000_000
        )
        self.assertEqual(hot_target["applicable_envelopes"], [])
        self.assertEqual(
            hot_target["selected_iterations"], 4_150_000_000
        )
        self.assertEqual(
            hot_target["selected_sources"],
            ["hot-8-20s-process-cpu-window"],
        )
        ...
        for mode in ("interpreter", "aot"):
            for threads, expected_duration, expected_source in (
                (2, 40_000_000_000, "atomic-2-4-40s-contention-window"),
                (4, 40_000_000_000, "atomic-2-4-40s-contention-window"),
                (8, 20_000_000_000, "atomic-8-20s-contention-window"),
            ):
                with self.subTest(
                    atomic_contention_window=(mode, threads)
                ):
                    atomic_target = bench.sizing_candidates_for_cell(
                        mode, "atomic", threads, 1_000_000, 1_000_000_000
                    )
                    self.assertEqual(
                        atomic_target["selected_iterations"],
                        expected_duration // 1_000_000_000 * 1_000_000,
                    )
                    self.assertEqual(
                        atomic_target["selected_sources"],
                        [expected_source],
                    )
                    self.assertEqual(
        ...
        for mode in ("interpreter", "aot"):
            self.assertEqual(
                bench.projected_duration_floor_for_cell(mode, "hot", 8),
                20_000_000_000,
            )
        self.assertEqual(
            bench.projected_duration_floor_for_cell("aot", "atomic", 1),
        ...
        for mode, workload, threads in (
            ...
            with self.subTest(cell=(mode, workload, threads)):
                candidates = bench.sizing_candidates_for_cell(
                    mode, workload, threads, 1_500_000, 2_000_000_000
                )
                self.assertEqual(candidates["applicable_envelopes"], [])
                self.assertEqual(
                    candidates["selected_iterations"],
                    candidates["general_rounded_iterations"],
                )
    ...
    def test_wait_attempt_envelope_counts_and_later_projections(self) -> None:
        ...
        for (
            ...
            later_elapsed,
            later_iterations,
            expected_projected_later,
        ) in attempts:
            with self.subTest(pilot_elapsed=pilot_elapsed):
                candidates = bench.sizing_candidates_for_cell(
                    "aot",
                    "wait-notify",
                    1,
                ...
                self.assertEqual(candidates["selected_iterations"], selected)
                projected_later = bench.ceil_div(
                    later_elapsed * selected, later_iterations
                )
                self.assertEqual(projected_later, expected_projected_later)
                self.assertGreater(projected_later, 1_250_000_000)

    def test_wait_envelope_boundary_and_actual_floor_remain_fail_closed(
        self,
    ) -> None:
        retained = 9.723491826731898
        declared = 16
        margin = declared / retained - 1
        self.assertAlmostEqual(margin, 0.645499403415209, places=15)
        self.assertGreater(
            1_250_000_000 * declared / retained,
            1_250_000_000,
        )
        ...
        selected = candidates["selected_iterations"]
        just_over_envelope_elapsed = (
            20_000_000_000 * 1_000_000 // 16_000_001
        )
        self.assertLess(just_over_envelope_elapsed, 1_250_000_000)
        result = guest_result(
        ...
        with self.assertRaisesRegex(
            ...
            bench.parse_guest_result(
                json.dumps(result),
                bench.expected_result("wait-notify", 1, selected),
                1_250_000_000,
            )

    def test_hot8_process_cpu_window_remains_fail_closed(self) -> None:
        pilot_iterations = 450_000_000
        pilot_elapsed_ns = 2_171_788_000
        later_iterations = 399_000_000
        later_elapsed_ns = 1_228_191_000
        retained = (
        ...
        self.assertAlmostEqual(
            16 / retained - 1, 9.204884230312514, places=15
        )
        candidates = bench.sizing_candidates_for_cell(
            "aot", "hot", 8, pilot_iterations, pilot_elapsed_ns
        )
        self.assertEqual(candidates["general_rounded_iterations"], 4_150_000_000)
        self.assertEqual(candidates["selected_iterations"], 4_150_000_000)
        self.assertEqual(
        ...
        with self.assertRaisesRegex(
            ...
            bench.parse_guest_result(
                json.dumps(result),
                bench.expected_result(
                    "hot", 8, candidates["selected_iterations"]
                ),
                1_250_000_000,
            )

    def test_wait4_failure_general_envelope_remains_fail_closed(
        self,
    ) -> None:
        pilot_iterations = 32_000
        pilot_elapsed_ns = 4_553_145_000
        later_iterations = 13_600
        later_elapsed_ns = 1_122_137_000
        retained = (
            later_iterations / later_elapsed_ns
        ) / (
            pilot_iterations / pilot_elapsed_ns
        ...
        with self.assertRaisesRegex(
            ...
            bench.parse_guest_result(
                json.dumps(result),
                bench.expected_result(
                    "wait-notify", 4, candidates["selected_iterations"]
                ),
                1_250_000_000,
            )

    def test_atomic2_contention_window_remains_fail_closed(
        self,
    ) -> None:
        pilot_iterations = 40_000_000
        pilot_elapsed_ns = 4_775_669_000
        later_iterations = 16_200_000
        later_elapsed_ns = 1_205_846_000
        retained = (
            later_iterations / later_elapsed_ns
        ) / (
            pilot_iterations / pilot_elapsed_ns
        )
        self.assertAlmostEqual(retained, 1.6039742595654836, places=15)
        candidates = bench.sizing_candidates_for_cell(
            "interpreter",
            "atomic",
            2,
        ...
        self.assertEqual(
            candidates["general_required_iterations"], 335_031_595
        )
        self.assertEqual(
            candidates["general_rounded_iterations"], 336_000_000
        )
        self.assertEqual(candidates["selected_iterations"], 336_000_000)
        self.assertEqual(
            candidates["selected_sources"],
            ["atomic-2-4-40s-contention-window"],
        )
        self.assertEqual(
            bench.ceil_div(
                later_elapsed_ns * candidates["selected_iterations"],
        ...
        just_over_contention_envelope_elapsed = (
            bench.projected_duration_floor_for_cell(
                "interpreter", "atomic", 2
            )
            * 1_000_000
            // 32_000_001
        )
        result = guest_result(
            ...
            threads=2,
            iterations=candidates["selected_iterations"],
            elapsed_ns=just_over_contention_envelope_elapsed,
            overhead_ns=1,
        )
        with self.assertRaisesRegex(
            bench.TimingQualityError, "below required"
            ...
            bench.parse_guest_result(
                json.dumps(result),
                bench.expected_result(
                    "atomic", 2, candidates["selected_iterations"]
                ),
                1_250_000_000,
            )

    def test_aot_atomic2_contention_window_remains_fail_closed(
        self,
    ) -> None:
        ...
        self.assertEqual(
            candidates["general_required_iterations"], 1_258_632_559
        )
        self.assertEqual(
            candidates["general_rounded_iterations"], 1_260_000_000
        )
        self.assertEqual(candidates["selected_iterations"], 1_260_000_000)
        self.assertEqual(
            candidates["selected_sources"],
            ["atomic-2-4-40s-contention-window"],
        )
    ...
    def test_sizing_downsizes_long_pilot_without_a_pilot_count_floor(self) -> None:
        report = make_report()
        plan = report["plan"]
        pilots = copy.deepcopy(plan["sizing"]["resolved"]["pilots"])
        target_pilots = [
        ...
        for pilot in target_pilots:
            pilot["guest_elapsed_ns"] = 30_000_000_000
            pilot["elapsed_ns"] = pilot["guest_elapsed_ns"]
            pilot["raw_guest_elapsed_ns"] = (
                pilot["guest_elapsed_ns"] + pilot["timing_overhead_ns"]
            )
        ...
        self.assertEqual(
            ...
            max(
                cell["general_rounded_iterations"],
                *[
                    item["rounded_iterations"]
                    for item in cell["applicable_envelopes"]
                ],
            ),
        )
        self.assertLess(
    ...
    def test_non_three_digit_cli_pilot_rounds_after_exact_ceiling(self) -> None:
        args = bench.parse_args(
            [
                "--no-budget",
                "--modes",
        ...
        plan = report["plan"]
        pilots = copy.deepcopy(plan["sizing"]["resolved"]["pilots"])
        order = copy.deepcopy(plan["sizing"]["pilot_order"])
        for expected, pilot in zip(order, pilots, strict=True):
            if (
                pilot["mode"] == "aot"
                and pilot["workload"] == "hot"
                and pilot["threads"] == 1
            ):
                expected["iterations"] = override
                pilot["iterations"] = override
                pilot["operations"] = bench.expected_result(
                    "hot", 1, override
                )["operations"]
                pilot["guest_elapsed_ns"] = (
                    2 * bench.PROJECTED_EVIDENCE_MINIMUM_NS
                )
                pilot["elapsed_ns"] = pilot["guest_elapsed_ns"]
                pilot["raw_guest_elapsed_ns"] = (
                    pilot["guest_elapsed_ns"] + pilot["timing_overhead_ns"]
        ...
        selected, _ = bench.resolve_one_shot_sizing(
            pilot_records=pilots,
            pilot_order=order,
            modes=tuple(plan["modes"]),
            thread_counts=tuple(plan["thread_counts"]),
    ...
    def test_full_authoritative_sizing_across_host_rates(self) -> None:
        ...
        envelope_provenance = provenance[
            "wait_notify_1_cell_specific_envelope"
        ]
        self.assertEqual(
        ...
        self.assertEqual(
            [
                item["sha256"]
                for item in envelope_provenance["evidence"]["diagnoses"]
            ],
            [
                "35540838a350d2e4e8bb9062dedfb733701702e24ffa044ef5dc1097282dcfe2",
                "8dd255b27a85811c918aabb9bfa75630e4de885591b1f50f0260f0e09273cda4",
                "1335e121e7386f18eb5970b3717237bc478d8074d9e576c7652eaa73daa33833",
                "1c8f13621986f4678999a19a4104dfdcd938642f837aae666d97ca101b6b0173",
                "8e8c9c7f0075ac7c9e36917dde73294a415f6756784107b06fb0055317035496",
            ],
        )
        self.assertEqual(
            envelope_provenance["evidence"][
                "retained_maximum_ordered_acceleration"
            ],
            9.723491826731898,
        )
        self.assertAlmostEqual(
            envelope_provenance["evidence"]["declared_margin"],
            0.645499403415209,
            places=15,
        )
        self.assertEqual(
            envelope_provenance["evidence"]["projection_profile"],
            {"warmups": 2, "samples": 10},
        )
        for attempt in envelope_provenance["evidence"]["attempts"]:
            candidates = bench.sizing_candidates_for_cell(
                "aot",
                "wait-notify",
                1,
                attempt["pilot_iterations"],
                attempt["fastest_pilot_elapsed_ns"],
            )
            self.assertEqual(
                candidates["general_rounded_iterations"],
                attempt["general_selected_iterations"],
            )
            self.assertEqual(
                candidates["selected_iterations"],
                attempt["selected_iterations"],
            )
            self.assertEqual(
                bench.ceil_div(
                    attempt["later_elapsed_ns"]
                    * attempt["selected_iterations"],
                    attempt["later_iterations"],
                ),
                attempt["projected_later_elapsed_ns"],
            )
            self.assertAlmostEqual(
                (
                    attempt["later_iterations"]
                ...
                attempt["measurement_to_fastest_pilot_ratio"],
                places=15,
            )
        self.assertEqual(
            max(
        ...
        for attempt in general_envelope["evidence"]["attempts"]:
            required, selected = bench.selected_iterations_from_elapsed(
                attempt["pilot_iterations"],
                attempt["fastest_pilot_elapsed_ns"],
            )
            self.assertLessEqual(required, selected)
            self.assertEqual(
                selected,
                attempt["selected_iterations"],
            )
            self.assertGreater(
                attempt["selected_iterations"],
                attempt["previous_selected_iterations"],
            )
            self.assertEqual(
                bench.ceil_div(
                    attempt["later_elapsed_ns"]
            ...
            self.assertAlmostEqual(
                ...
                attempt["ordered_rate_acceleration"],
                places=15,
            )
        self.assertEqual(
            max(
        ...
        self.assertEqual(
            ...
            - reserve_evidence["record_host_wall_elapsed_ns"],
            reserve_evidence["unaccounted_step_wall_ns"],
        )
        self.assertAlmostEqual(
            reserve_admission["job_non_benchmark_reserve_ns"]
            / reserve_evidence["unaccounted_step_wall_ns"]
        ...
        self.assertAlmostEqual(
            (
                reserve_admission["job_non_benchmark_reserve_ns"]
                + reserve_admission["auxiliary_invocation_budget_ns"]
            )
            / reserve_evidence["unaccounted_step_wall_ns"]
            - 1,
            reserve_evidence["combined_allowance_margin"],
            places=15,
        )
        self.assertEqual(
            reserve_admission["hard_preadmission_benchmark_ns"]
        ...
        for label, rate_numerator, rate_denominator in rates:
            ...
            with self.subTest(rate=label):
                ...
                projected_guest = []
                hot8_projected_guest = []
                spawn_projected_guest = []
                wait_projected_guest = []
                atomic_40s_projected_guest = []
                atomic_20s_projected_guest = []
                for item in resolved["projections"]:
                    ...
                    destination = (
                        ...
                        else hot8_projected_guest
                        if (
                            pilot["workload"] == "hot"
                            and pilot["threads"] == 8
                        )
                        else atomic_40s_projected_guest
                        if (
                            pilot["workload"] == "atomic"
                            and pilot["threads"] in (2, 4)
                        )
                        else atomic_20s_projected_guest
                        if (
                            pilot["workload"] == "atomic"
                            and pilot["threads"] == 8
                        )
                ...
                self.assertGreaterEqual(
                    min(hot8_projected_guest), 20_000_000_000
                )
                self.assertLess(
                    max(hot8_projected_guest),
                    20_000_000_000 * 101 // 100,
                )
                self.assertGreaterEqual(
                ...
                self.assertLess(
                    max(wait_projected_guest),
                    20_000_000_000 * 101 // 100,
                )
                self.assertGreaterEqual(
                    min(atomic_40s_projected_guest), 40_000_000_000
                )
                self.assertLess(
                    max(atomic_40s_projected_guest),
                    40_000_000_000 * 101 // 100,
                )
                self.assertGreaterEqual(
                    min(atomic_20s_projected_guest), 20_000_000_000
                )
                self.assertLess(
                    max(atomic_20s_projected_guest),
                    20_000_000_000 * 101 // 100,
                )
                self.assertLess(
    ...
    def test_sizing_fails_caps_and_per_invocation_projection(self) -> None:
        ...
        with mock.patch.dict(
            ...
            self.assertEqual(
                bench.effective_sizing_cap("wait-notify", 1),
                bench.INT32_MAX,
            )
        report = make_report()
        plan = report["plan"]
    ...
    def test_pilot_duration_and_job_bounds_are_hard(self) -> None:
        ...
        progress = bench.pilot_progress_bound(
            pilot_records=[],
            pilot_order=order,
    ...
    def test_pilot_progress_bound_charges_cell_envelopes(self) -> None:
        _, order, _ = authoritative_sizing_inputs(
            bench.PROJECTED_EVIDENCE_MINIMUM_NS,
            0,
        )
        progress = bench.pilot_progress_bound(
        ...
        self.assertEqual(
            progress["remaining_pilot_bound_ns"],
            88 * 33_000_000_000,
        )
        self.assertEqual(
            progress["minimum_evidence_bound_ns"],
            expected_evidence,
        )
        self.assertEqual(
            progress["earliest_complete_bound_ns"],
    ...
    def test_authoritative_actual_projection_over_261_minutes_fails(self) -> None:
        pilots, order, thread_counts = authoritative_sizing_inputs(
            bench.PROJECTED_EVIDENCE_MINIMUM_NS,
            0,
        )
        with self.assertRaisesRegex(
            bench.HarnessError,
            "261-minute benchmark bound|benchmark share",
        ):
            bench.resolve_one_shot_sizing(
                pilot_records=pilots,
                pilot_order=order,
    ...
    def test_pr_1016_old_identity_is_rejected_for_reports_and_cohorts(self) -> None:
        report = make_report()
        report["metadata"]["measurement_plan_version"] = 3
        with self.assertRaisesRegex(
            BenchmarkDataError, "measurement_plan_version"
        ):
            bench.validate_report(report)

        reports = make_paired_cohort_reports()
        reports[-1][1]["metadata"]["measurement_plan_version"] = 3
        with self.assertRaisesRegex(
            BenchmarkDataError, "measurement_plan_version"
        ):
    ...
    def test_report_replays_sizing_after_plan_hash_tampering(self) -> None:
        ...
        envelope = make_report()
        envelope["plan"]["sizing"]["algorithm"]["cell_envelopes"][0][
            "quality_floor_ns"
        ] -= 1
        rehash(envelope)
        with self.assertRaisesRegex(
    ...
    def test_workflow_enables_trusted_preflight_and_retains_failures(self) -> None:
        ...
        for section in (hosted, trusted_x86, trusted_arm):
            self.assertIn("timeout-minutes: 330", section)
    ...
    def test_actual_samples_keep_exact_interval_and_overhead_gates(self) -> None:
        expected = bench.expected_result("atomic", 1, 10)
        minimum = int(bench.MIN_TIMED_INTERVAL_MS * 1_000_000)
        maximum_overhead = (minimum - 1) // 99
        accepted = guest_result(
            elapsed_ns=minimum,
            overhead_ns=maximum_overhead,
        )
        self.assertEqual(
            bench.parse_guest_result(
                json.dumps(accepted),
        ...
        with self.assertRaisesRegex(bench.TimingQualityError, "below 1%"):
            bench.parse_guest_result(
                json.dumps(
                    guest_result(
                        elapsed_ns=minimum,
                        overhead_ns=maximum_overhead + 1,
                    )
                ),
                expected,
                minimum,
    ...
    def test_fixture_hashes_and_schema_are_pinned(self) -> None:
        ...
        self.assertEqual(
            sizing_algorithm["version"]["const"],
            bench.SIZING_ALGORITHM_VERSION,
        )
        self.assertEqual(
            sizing_algorithm["formula"]["const"],
            bench.SIZING_FORMULA,
        )
        self.assertEqual(
        ...
        self.assertIn(
            ...
            schema["$defs"]["sizing_plan"]["properties"]["resolved"][
                "properties"
            ]["cells"]["items"]["required"],
        )
        readme = (
            ROOT / "tests" / "benchmarks" / "wasi-threads" / "README.md"
        ).read_text(encoding="UTF-8")
        self.assertIn(f"`{bench.SIZING_FORMULA}`", readme)
        self.assertIn(
        ...
        self.assertIn("--timeout-seconds 432000", readme)
